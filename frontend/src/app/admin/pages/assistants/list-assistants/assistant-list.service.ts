...
import { Table, SearchResult } from './assistant-list.model';
 ...
 */
function sort(tables: Table[], column: keyof Table, direction: string): Table[] {
  if (direction === '') {
    return tables;
  } else {
...
export class AssistantListService {
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let tables = sort(tableData, sortColumn as keyof Table, sortDirection);
